...
Three measured runs per variant cannot establish small differences. The 1.00× cache result and the 1.13× Q6 AQE result are within run-to-run variance and should be read as "no clear effect", not as precise ratios. Operating-system caching is uncontrolled; the warm-up absorbs cold reads, but later runs may still benefit unevenly. Two baselines drift across their three runs, `q1_broadcast_zones` falling (10.13, 8.05, 7.27 s) and `q4_aqe` rising (20.01, 28.15, 28.45 s), which indicates that a single warm-up does not fully stabilize these queries. All figures come from one machine, one data volume and a local Spark master; cluster behaviour, where shuffle crosses the network, is untested.
