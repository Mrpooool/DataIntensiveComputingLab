...
def load_calendar_coverage(
    ...
    *,
    ...
    Incremental updates extend the window; the integration snapshot they publish
    carries it as ``coverage_window``, which wins over the configured one.
    """
    contract = (snapshot or {}).get("coverage_window") or load_dataset_config("taxi", config_path)
    bounds = []
